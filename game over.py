import time

def clear():
    print(' \n' * 75)

def game_over():
  clear()
  print('''
    .oooooo.                                          

   ''')
  time.sleep(0.3)
  clear()
  print('''
    .oooooo.                                          
   d8P'  `Y8b                                         
   
   ''')
  time.sleep(0.3)
  clear()
  print('''
    .oooooo.                                          
   d8P'  `Y8b                                         
  888            .oooo.   ooo. .oo.  .oo.    .ooooo.  

   ''')
  time.sleep(0.3)
  clear()
  print('''
    .oooooo.                                          
   d8P'  `Y8b                                         
  888            .oooo.   ooo. .oo.  .oo.    .ooooo.  
  888           `P  )88b  `888P"Y88bP"Y88b  d88' `88b 
   
   ''')
  time.sleep(0.3)
  clear()
  print('''
    .oooooo.                                          
   d8P'  `Y8b                                         
  888            .oooo.   ooo. .oo.  .oo.    .ooooo.  
  888           `P  )88b  `888P"Y88bP"Y88b  d88' `88b 
  888     ooooo  .oP"888   888   888   888  888ooo888 
    
   ''')
  time.sleep(0.3)
  clear()
  print('''
    .oooooo.                                          
   d8P'  `Y8b                                         
  888            .oooo.   ooo. .oo.  .oo.    .ooooo.  
  888           `P  )88b  `888P"Y88bP"Y88b  d88' `88b 
  888     ooooo  .oP"888   888   888   888  888ooo888 
  `88.    .88'  d8(  888   888   888   888  888    .o 
   
   ''')
  time.sleep(0.3)
  clear()
  print('''
    .oooooo.                                          
   d8P'  `Y8b                                         
  888            .oooo.   ooo. .oo.  .oo.    .ooooo.  
  888           `P  )88b  `888P"Y88bP"Y88b  d88' `88b 
  888     ooooo  .oP"888   888   888   888  888ooo888 
  `88.    .88'  d8(  888   888   888   888  888    .o 
   `Y8bood8P'   `Y888""8o o888o o888o o888o `Y8bod8P' 
   
   ''')
  time.sleep(0.3)
  clear()
  print('''
    .oooooo.                                          
   d8P'  `Y8b                                         
  888            .oooo.   ooo. .oo.  .oo.    .ooooo.  
  888           `P  )88b  `888P"Y88bP"Y88b  d88' `88b 
  888     ooooo  .oP"888   888   888   888  888ooo888 
  `88.    .88'  d8(  888   888   888   888  888    .o 
   `Y8bood8P'   `Y888""8o o888o o888o o888o `Y8bod8P' 
                                                      
                                                      
                                                      
    .oooooo.                                  
    
   ''')
  time.sleep(0.3)
  clear()
  print('''
    .oooooo.                                          
   d8P'  `Y8b                                         
  888            .oooo.   ooo. .oo.  .oo.    .ooooo.  
  888           `P  )88b  `888P"Y88bP"Y88b  d88' `88b 
  888     ooooo  .oP"888   888   888   888  888ooo888 
  `88.    .88'  d8(  888   888   888   888  888    .o 
   `Y8bood8P'   `Y888""8o o888o o888o o888o `Y8bod8P' 
                                                      
                                                      
                                                      
    .oooooo.                                  
   d8P'  `Y8b                                 
   
   ''')
  time.sleep(0.3)
  clear()
  print('''
    .oooooo.                                          
   d8P'  `Y8b                                         
  888            .oooo.   ooo. .oo.  .oo.    .ooooo.  
  888           `P  )88b  `888P"Y88bP"Y88b  d88' `88b 
  888     ooooo  .oP"888   888   888   888  888ooo888 
  `88.    .88'  d8(  888   888   888   888  888    .o 
   `Y8bood8P'   `Y888""8o o888o o888o o888o `Y8bod8P' 
                                                      
                                                      
                                                      
    .oooooo.                                  
   d8P'  `Y8b                                 
  888      888 oooo    ooo  .ooooo.  oooo d8b 
    
   ''')
  time.sleep(0.3)
  clear()
  print('''
    .oooooo.                                          
   d8P'  `Y8b                                         
  888            .oooo.   ooo. .oo.  .oo.    .ooooo.  
  888           `P  )88b  `888P"Y88bP"Y88b  d88' `88b 
  888     ooooo  .oP"888   888   888   888  888ooo888 
  `88.    .88'  d8(  888   888   888   888  888    .o 
   `Y8bood8P'   `Y888""8o o888o o888o o888o `Y8bod8P' 
                                                      
                                                      
                                                      
    .oooooo.                                  
   d8P'  `Y8b                                 
  888      888 oooo    ooo  .ooooo.  oooo d8b 
  888      888  `88.  .8'  d88' `88b `888""8P 
    
   ''')
  time.sleep(0.3)
  clear()
  print('''
    .oooooo.                                          
   d8P'  `Y8b                                         
  888            .oooo.   ooo. .oo.  .oo.    .ooooo.  
  888           `P  )88b  `888P"Y88bP"Y88b  d88' `88b 
  888     ooooo  .oP"888   888   888   888  888ooo888 
  `88.    .88'  d8(  888   888   888   888  888    .o 
   `Y8bood8P'   `Y888""8o o888o o888o o888o `Y8bod8P' 
                                                      
                                                      
                                                      
    .oooooo.                                  
   d8P'  `Y8b                                 
  888      888 oooo    ooo  .ooooo.  oooo d8b 
  888      888  `88.  .8'  d88' `88b `888""8P 
  888      888   `88..8'   888ooo888  888     
   
   ''')
  time.sleep(0.3)
  clear()
  print('''
    .oooooo.                                          
   d8P'  `Y8b                                         
  888            .oooo.   ooo. .oo.  .oo.    .ooooo.  
  888           `P  )88b  `888P"Y88bP"Y88b  d88' `88b 
  888     ooooo  .oP"888   888   888   888  888ooo888 
  `88.    .88'  d8(  888   888   888   888  888    .o 
   `Y8bood8P'   `Y888""8o o888o o888o o888o `Y8bod8P' 
                                                      
                                                      
                                                      
    .oooooo.                                  
   d8P'  `Y8b                                 
  888      888 oooo    ooo  .ooooo.  oooo d8b 
  888      888  `88.  .8'  d88' `88b `888""8P 
  888      888   `88..8'   888ooo888  888     
  `88b    d88'    `888'    888    .o  888     
   
   ''')
  time.sleep(0.3)
  clear()
  print('''
    .oooooo.                                          
   d8P'  `Y8b                                         
  888            .oooo.   ooo. .oo.  .oo.    .ooooo.  
  888           `P  )88b  `888P"Y88bP"Y88b  d88' `88b 
  888     ooooo  .oP"888   888   888   888  888ooo888 
  `88.    .88'  d8(  888   888   888   888  888    .o 
   `Y8bood8P'   `Y888""8o o888o o888o o888o `Y8bod8P' 
                                                      
                                                      
                                                      
    .oooooo.                                  
   d8P'  `Y8b                                 
  888      888 oooo    ooo  .ooooo.  oooo d8b 
  888      888  `88.  .8'  d88' `88b `888""8P 
  888      888   `88..8'   888ooo888  888     
  `88b    d88'    `888'    888    .o  888     
   `Y8bood8P'      `8'     `Y8bod8P' d888b  
   ''')
  time.sleep(5)
  clear()
  time.sleep(2)
  print('''
  	       -/oydmmNNNNNmdyo/-                
               :ymNNNNNNNNNNNNNNNNNNNy/`            
            `+mNNNNNNNNNNNNNNNNNNNNNNNNmo`          
           -dNNNNNNNNNNNNNNNNNNNNNNNNNNNNd-         
          -mNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN-        
          dNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNd        
         -NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN:       
         /NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN/       
         :NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN:       
         .NNNNNNms///+yNNNNNNNNdo/:/ohNNNNNN`       
          hNNNNm.      /NNNNNNh`     `yNNNNo        
          .dNNNy       .NNNNNN/       /NNNh`        
           .dNNd       /NNNNNNs       sNNd.         
            .mNNy:.`.-+mNNmmNNNo-.`.-sNNm.          
             oNNNNmmmmNNNh-.yNNNNmmmNNNNs           
             -NNNNNNNNNNo`  `/NNNNNNNNNN:           
              :shmNNNNNN+-::-/NNNNNNmhs/            
                `.:ohNNNNNNNNNNNNho:.`              
                 -y+ sNNNNNNNNNNy`oy.               
                 `NN+`-/++++++/:`oNd                
                  oNNh/-......-/hNN/                
                  `:hNNNmmmmmNNNmy:                 
    .::.`            -dNNNNNNNNh-             `--`  
   `mNNNh/`           `:shddhs:`            -odNNd` 
   .NNNNNNd+`                             -yNNNNNN` 
   `mNNNNNNNd/.`                      `.:yNNNNNNNm` 
  `yNNNNNNNNNNNdyo:.`             `-/shmNNNNNNNNNNh.
  hNNNNNdydmmNNNNNNNhs/.`    ``:oymNNNNNNmmhydNNNNNh
  :yhyo-   `.:+sdmNNNNNNds/:ohmNNNNNmds+:.   `-ohdd+
                 .:+hmNNNNNNNNNNmho:.               
                   `-sNNNNNNNNNNs-`                 
                `/smNNNNNmyhmNNNNNds:`              
             `/yNNNNNNmo:`  `/ymNNNNNmy/`           
      ``.-/+yNNNNNNms:         `:ymNNNNNNy+-.```    
  `/yhmNNNNNNNNNNh/`              `/hNNNNNNNNNNdyo- 
  dNNNNNNNNNNNNs.                    -sNNNNNNNNNNNNy
  yNNNNNNNNNNN.                        -NNNNNNNNNNNy
   `://yNNNNNy                          +NNNNNms+:` 
        :hNms`                           /dNd+`     

  	''')
  time.sleep(5)
game_over()
input()